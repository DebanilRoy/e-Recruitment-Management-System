...
export default function VerifyApplication() {
    ...
    function getApplicationDetails(event) {
        event.preventDefault()
        $.ajax({
            type: "POST",
            url: "http://localhost:8000/src/verifyApplications/getRecruitmentDetails.php",
            data: JSON.stringify(recruitmentID),
            success: (details) => {
                $.ajax({
                    type: "POST",
                    url: "http://localhost:8000/src/verifyApplications/getApplications.php",
                    data: JSON.stringify(recruitmentID),
    ...
    function getApplicationBio(applicationID) {
        $.ajax({
            type: "POST",
            url: "http://localhost:8000/src/verifyApplications/getApplicationBio.php",
    ...
    async function verifyApplications(event) {
        ...
        if (confirmVerify) {
            $.ajax({
            type: "POST",
            url: ("http://localhost:8000/src//verifyApplications/verifyApplications.php"),
    ...
    function applicationBioClose() {
        setOpenApplicationBio(false)
    }
        ...
        <>
            <div className="bodyDiv">
                ...
                <div className="divRecruitmentInfo"> 
                    <form onSubmit={(event) => {getApplicationDetails(event);}}>
                    ...
                    {(recruitmentDetails) ? (
                        <div className="divRecruitmentDetailsLabel verifyApplicationsLabel">
                            ...
                            <p>Applications Checked : <span>{recruitmentDetails.checked}</span></p>
                            <p>Application Verified : <span>{recruitmentDetails.verified}</span></p>
                        </div>
                    ) : null}
                </div>     
